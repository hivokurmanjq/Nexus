...
import { NEXUS_ANALYTICS, NEXUS_CORE } from './nexus/core.js';
...
async function handleWebhook(request, env, ctx, url) {
  ...
  if (!update) return new Response('Bad JSON', { status: 400, headers: SECURITY_HEADERS });

  if (update.update_id) {
    const dk = `u:${update.update_id}`;
    const cache = new Cache(env);
    if (await cache.get(dk)) return new Response('OK', { headers: SECURITY_HEADERS });
  ...
  ctx.waitUntil((async () => {
    try {
      ...
      Log.error('handler', { err: err.message, stack: err.stack });
    }
  })());
...
async function handleSetup(url, env) {
  ...
  if (action === 'set') {
    ...
    return Response.json({ action: 'set', webhookUrl, response: res }, { headers: SECURITY_HEADERS });
  }
  if (action === 'info') {
    return Response.json(await api.getWebhookInfo(), { headers: SECURITY_HEADERS });
  }
  if (action === 'schema') {
    const result = await nexusInstallSchema(env);
    return Response.json(
      { action: 'schema', ok: result.every(Boolean), statements: result.length, result },
      { headers: SECURITY_HEADERS }
    );
  }
  if (action === 'bootstrap') {
    const commands = [
      { command: 'start', description: 'باز کردن NEXUS' },
      { command: 'panel', description: 'مرکز فرماندهی گروه' },
      { command: 'help', description: 'راهنمای NEXUS' },
      { command: 'status', description: 'وضعیت امنیت' },
      { command: 'ai', description: 'دستیار هوشمند' },
    ];
    const results = { commands: await api.setMyCommands(commands), menu: null };
    if (env.MINI_APP_URL) results.menu = await api.setChatMenuButton(null, 'NEXUS', env.MINI_APP_URL);
    return Response.json({ action: 'bootstrap', results }, { headers: SECURITY_HEADERS });
  }
  return Response.json({
    usage: {
...
async function handleDashboardApi(url, env) {
  ...
  if (!token) return Response.json({ok:false,error:'missing_token'},{status:401,headers:SECURITY_HEADERS});
  const db = new Database(env);
  const t = await db.validateToken(token);
  if (!t) return Response.json({ok:false,error:'invalid_token'},{status:403,headers:SECURITY_HEADERS});
  const chats = await db.all(`SELECT c.* FROM chats c WHERE c.is_active=1 ORDER BY c.title LIMIT 100`);
  const out=[];
  for (const c of chats) {
    const stats=await NEXUS_ANALYTICS.overview(db,c.id);
    const threats=await db.nexusThreatSummary(c.id).catch(()=>({total:0,watch:0,high:0,critical:0}));
    out.push({id:c.id,title:c.title,type:c.type,stats,threats});
  }
  return Response.json(
    {ok:true,product:NEXUS_CORE,owner:t.user_id,chats:out},
...
async function handleDashboardPage(url, env) {
  const token = url.searchParams.get('token');
  if (!token) return new Response('Missing token', { status: 401, headers: SECURITY_HEADERS });
  const db = new Database(env);
  const t = await db.validateToken(token);
  if (!t) return new Response('Invalid or expired token', { status: 403, headers: SECURITY_HEADERS });
...
export async function route(request, env, ctx) {
  try {
    ...
    if (url.pathname === '/app') {
      return new Response(renderNexusApp(env), {
        headers: { 'Content-Type': 'text/html; charset=utf-8', ...SECURITY_HEADERS },
      });
    }
